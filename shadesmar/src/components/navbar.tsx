...
export function Navbar() {
    ...
    return (
        <nav className="w-full" style={{ background: "#0d0d12", borderBottom: "1px solid rgba(99,210,210,0.15)", fontFamily: "'Raleway', sans-serif" }}>
            <div className="mx-auto flex max-w-6xl items-center justify-between px-5" style={{ height: "64px" }}>
                ...
                <ul className="hidden md:flex items-center gap-1 list-none m-0 p-0">
                    {toursDictionary[location.pathname] && (
                        <li>
                            <button id="TourButton" onClick={handleTour} title="Start tour"
                    ...
                    {navLinks.map(({ to, label }) => (
                        <li key={to}>
                            <Link to={to} className="block rounded-md px-3 py-1.5 text-sm font-medium tracking-wide no-underline transition-all"
                    ...
                    {isAdmin && (
                        <li>
                            <Link to="/admin" className="block rounded-md px-3 py-1.5 text-sm font-medium tracking-wide no-underline transition-all"
                ...
                {/* Hamburger */}
                <button className="flex md:hidden items-center justify-center rounded-md p-2 transition-all"
                    onClick={() => setMenuOpen(o => !o)}
                    aria-label="Toggle menu"
                    aria-expanded={menuOpen}
                    style={{ background: "none", border: "1px solid rgba(61,214,214,0.2)", color: "rgba(61,214,214,0.7)", cursor: "pointer" }}>
            ...
            {/* Mobile menu */}
            {menuOpen && (
                <div className="md:hidden flex flex-col pb-3" style={{ borderTop: "1px solid rgba(99,210,210,0.1)" }}>
                    {toursDictionary[location.pathname] && (
                        <div style={{ padding: "8px 1.25rem", borderBottom: "1px solid rgba(99,210,210,0.08)", marginBottom: "4px" }}>
                    ...
                    {navLinks.map(({ to, label }) => (
                        <Link key={to} to={to}
                            ...
                            style={{ color: location.pathname === to ? "#3dd6d6" : "rgba(200,210,230,0.7)" }}>
                            {label}
                        </Link>
                    ))}
                    {isAdmin && (
